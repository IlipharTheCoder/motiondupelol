...
**"Todoist is master," rows can now be hard-deleted, not just marked `'discarded'`/`'completed'` (added 2026-07-25, two-way sync — `synced_tasks`'s entry below has the full design):** a Todoist-tracked task that vanishes from Todoist while still `'unscheduled'` on our side gets `DELETE`d outright now (was `status: 'completed'` before this change); one that's `'scheduled'` still goes through the normal calendar-delete review queue first, but once that's approved the linked row is hard-deleted too (`lib/proposedChanges.ts`'s `applyProposedChange`, scoped to `source_system: 'todoist'` deletes only — any other delete reason still just marks the row `'completed'`, unchanged). A row already `'completed'`/`'discarded'` when the sync observes the vanish is left alone — this is what stops our *own* `completeTask()` → Todoist-push from re-triggering a spurious hard-delete or calendar-delete proposal on the very next sync.
...
Dedup/mapping table for the Todoist and Canvas syncs (Phase 4 item 12, `lib/canvasSync.ts`), mirroring `synced_events`/`calendar_sync_state` (section 2a) — same pattern, not reinvented. Keyed by `{source_system, source_id}` rather than a Supabase-generated id, same reasoning as `event_metadata`. **As of the 2026-07-25 two-way Todoist sync, this table is also written by the *push* direction** — `lib/todoistWrite.ts`'s `pushNewTaskToTodoist` inserts a row (with `task_id` already known, unlike the Todoist-origin case where it starts `null`) the moment a manually-created task is pushed out to Todoist, so later completions/edits know where to push and the next `runTodoistSync()` diff recognizes the task as already-tracked rather than proposing it as "new."

| Column | Type | Notes |
|---|---|---|
| `source_system` | `text` | Part of composite PK |
| `source_id` | `text` | Part of composite PK — the source system's own task id |
| `proposed_change_id` | `uuid` | The `create` proposal made for this task while it's still unresolved (`references proposed_changes(id)`). Set at intake, read by `applyProposedChange` to link the resulting `tasks` row back once approved. `null` for a row created by the push direction (a manually-created task never goes through the review queue in the first place) |
| `task_id` | `uuid` | The resulting `tasks` row, once approved (`references tasks(id)`). `null` while a Todoist-origin intake proposal is still pending/failed; always set immediately for a push-direction row |
| `source_updated_at` | `timestamptz` | The source system's own last-modified signal, informational only, still not read anywhere. For Todoist specifically, the `/api/v1` API exposes no true "last modified" timestamp, so this is populated from the task's `added_at`/the current time as a best-effort placeholder. **Superseded as the mechanism for propagating post-import edits** — that's no longer "out of scope"; see below |
| `created_at` | `timestamptz` | Default `now()` |
| `updated_at` | `timestamptz` | Default `now()` |

**Post-import edit propagation, Todoist → us (added 2026-07-25, `lib/todoistSync.ts`'s `syncEditFromTodoist`):** every sync run now diffs each already-tracked Todoist task's current `content`/`description`/`due`/`duration` against whichever of `tasks` or the still-open `proposed_changes` row this maps to, and overwrites ours if different — "Todoist is master," per the user's own framing. Scoped to those four fields only; `priority`/`tags` are deliberately never touched by this direction, same reasoning intake itself already used to leave them unset (Todoist can't tell us how you want to prioritize/organize your own work). Canvas sync was not given the equivalent treatment — its own "create-once limitation" is unchanged, out of scope for this pass (not asked for; Canvas sync is also still non-functional/on-hiatus regardless — see `backend-api-reference.md`'s `POST /api/canvas/sync`).

**Push direction, us → Todoist (added 2026-07-25, `lib/todoistWrite.ts`), real-time (same request, not batched to the next sync):**
- **New tasks** — `lib/tasksWrite.ts`'s `createTask` (used by both `POST /api/tasks` and chat's `create_task`) pushes a new Todoist task after inserting ours.
- **Completions** — `lib/aiTasks.ts`'s `completeTask` pushes a Todoist `close` if the task is mapped.
- **Edits** — the new `lib/tasksWrite.ts`'s `updateTask` (behind the new `PATCH /api/tasks/{id}`) pushes title/description/deadline/duration/priority if the task is mapped.
- All three are **best-effort** — a Todoist API failure never undoes the action that already succeeded on our side. A disclosed, unresolved risk this creates: if a push-edit call fails silently, Todoist's copy stays stale, and the *next* sync's Todoist-wins edit-detection (above) will then overwrite our edit right back to the old value — an accepted tradeoff, not a hidden one.
- Only tasks with `source_system: 'manual'` are ever pushed this way (i.e. direct API or chat's `create_task`, which both go through the same `createTask`) — Canvas-sourced tasks are never bounced back out to Todoist.
