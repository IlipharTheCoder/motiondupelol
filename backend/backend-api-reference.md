...
**What it does:** Phase 5 — the NL chat layer. **Completely rebuilt 2026-07-25** (per an explicit user decision) from a 33-tool surface down to exactly **two abilities**, because the prior surface was too large for `claude-haiku-4-5` to reliably keep track of, then **grown to eight abilities the same day** to add task assignment, direct task creation, completion, and habit planning:
