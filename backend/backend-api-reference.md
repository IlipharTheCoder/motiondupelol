...
**What it does:** on-demand Todoist task sync (`architecture-plan.md` section 4a) — fetches your active Todoist tasks (`TODOIST_API_TOKEN`, a personal access token) and diffs the full list against `synced_tasks`. This one endpoint only covers the **Todoist → us** direction; the reverse (**us → Todoist**, real-time, not batched here) happens separately, at the point a task is created/edited/completed in our own system — see `backend-schema.md`'s `synced_tasks` entry for the full two-way design and `POST /api/tasks`/`PATCH /api/tasks/{id}`/`POST /api/tasks/{id}/complete` for where each push actually fires.

Every Todoist task with no `synced_tasks` row yet gets a `create` proposal into `proposed_changes` (`category: 'task'`, no `proposed_start`/`proposed_end`, `proposed_summary` = the task's title, `deadline` = its due date, `source_system: 'todoist'`) — this is the "add to the task list" shape described above, so nothing lands in `tasks` until you approve it (and, typically, set `priority`/`tags` first via `PATCH /api/proposed-changes/{id}`).

**A Todoist task already tracked (`skippedExisting`) is now also checked for edits (added 2026-07-25)** — if its title/description/due-date/duration changed in Todoist since intake, that overwrites our copy (the still-open proposal if not yet approved, or the `tasks` row if it is) — "Todoist is master." `priority`/`tags` are deliberately never touched by this direction. See `backend-schema.md`'s `synced_tasks` entry (`syncEditFromTodoist`) for the full reasoning; this closes what used to be an explicit, accepted "create-once" limitation.

**"Todoist is master" applies to vanished tasks too — updated 2026-07-25, genuinely destructive now, not just a status flip.** Tasks that drop out of Todoist's active list (completed or deleted there — the REST API doesn't distinguish; `checked`/`is_deleted` both just mean "not in the active list") get one of four handlings, checked in this order:
- **Already resolved on our side** (`status` already `'completed'`/`'discarded'`) — nothing further happens to the task; only the now-stale `synced_tasks` mapping is cleaned up. This is what stops our *own* completions (which push a Todoist close, see `POST /api/tasks/{id}/complete`) from re-triggering one of the destructive branches below on the very next sync.
...
- **Approved and still scheduled onto the calendar** — a `delete` proposal for that calendar event lands in the review queue, same as any other calendar change; approving it now hard-deletes the linked `tasks` row (`applyProposedChange`'s delete branch, scoped to `source_system: 'todoist'` — any other delete reason still just marks the row `'completed'`, unchanged, since deleting a scheduled task's event for an unrelated reason like rescheduling shouldn't destroy the task).
- **Approved, still active, never scheduled** — the `tasks` row is hard-deleted directly (changed 2026-07-25, was `status: 'completed'` before that, and `'discarded'` before that — see `POST /api/tasks/{id}/complete`'s own history). Explicitly requested: "if a [task] is gone in Todoist, assume it's done and can be safely removed from the list." **Canvas sync (`POST /api/canvas/sync` below) was not changed** and still marks its equivalent case `'discarded'`, never deletes — not an oversight, just out of scope for this change; the two sources are inconsistent with each other until Canvas gets the same treatment.

Same on-demand-only shape as `POST /api/calendar/sync` — nothing in this backend runs on a schedule yet. Only this Todoist→us direction is on-demand-only; the us→Todoist push direction is real-time (fires in the same request as the triggering action), a deliberate asymmetry confirmed directly with the user rather than assumed.
...
**Also pushes the new task to Todoist (added 2026-07-25, two-way sync, best-effort, real-time — same request):** `lib/todoistWrite.ts`'s `pushNewTaskToTodoist` creates the corresponding Todoist task and records the mapping in `synced_tasks` immediately. A Todoist API failure here is swallowed — it never fails this request, since the task already exists in our own system by that point. See `backend-schema.md`'s `synced_tasks` entry for the full two-way design (push direction, edit-sync direction, and the hard-delete change to `POST /api/todoist/sync` below).
...
**Request:** `application/json`, at least one of `title`/`description`/`deadline`/`priority`/`duration_minutes`/`tags` — same field set `POST /api/tasks` accepts at creation, now editable after the fact. `deadline: null` clears the deadline; same for `priority`.

**What it does:** edits a task's fields directly (`lib/tasksWrite.ts`'s `updateTask`) — no app UI calls this yet (the app is still view/create/complete only), it exists specifically so two-way Todoist sync has something to push an edit *from*. Also pushes the edit to Todoist (best-effort, real-time), same as creation — no-op if this task was never synced/pushed to Todoist in the first place.

**Response:** the updated `tasks` row
...
**Also pushes a close to Todoist (added 2026-07-25, two-way sync, best-effort, real-time)** if this task is mapped in `synced_tasks` — no-op otherwise. This is also what makes `POST /api/todoist/sync`'s vanished-task handling status-aware: completing a task here can itself be the reason it later vanishes from Todoist, and that sync run needs to recognize it's already resolved rather than reprocessing it.
...
**`lib/habitPlacement.ts`'s `planHabitPlacement` gained an internal-only `skipAutoApply` option (2026-07-25, no public route exposes it — same pattern as `createProposedChange`'s own option).** This route doesn't set it, so it keeps respecting `AUTO_APPLY_CATEGORIES` exactly as before; chat's `plan_habits` tool (`POST /api/chat`) always passes `true` instead, since the user was explicit that habit planning through chat must go through manual review regardless of that env var.
...
**What it does:** Phase 5 — the NL chat layer. **Completely rebuilt 2026-07-25** (per an explicit user decision) from a 33-tool surface down to exactly **two abilities**, because the prior surface was too large for `claude-haiku-4-5` to reliably keep track of, then **grown to nine abilities the same day** to add task assignment, direct task creation, completion, habit planning, and habit declaration:
...
- **`plan_habits`** — auto-fill this period's remaining habit occurrences, spaced out rather than clustered (`lib/habitPlacement.ts`'s `planHabitPlacement`, same function `POST /api/habits/plan` uses). No params — always every active habit against its own cadence period, same as the direct endpoint. Always creates `pending` proposals, one per occurrence — **unlike the direct API, this always forces `skipAutoApply: true`** (see below); the user was explicit that habit planning must go through the same review tree as everything else in chat, even though `POST /api/habits/plan` itself doesn't force that and still respects `AUTO_APPLY_CATEGORIES`.
- **`create_habit`** — declare a new recurring habit (`lib/habitsWrite.ts`'s `createHabit`, same function `POST /api/habits` uses). Applies immediately, no proposal — a habit declaration is a goal, not a calendar write, same category of action as `create_task`, distinct from `plan_habits`' calendar-writing occurrences. No editing/pausing an existing habit from chat yet (`PATCH /api/habits/{id}` still direct-API only).

Everything else from the prior surface (editing/pausing a habit, scheduling-rule declarations, batch/group operations, bulk-edit, recurring series, relocate, reschedule/rebalance, focus/buffer auto-placement, approve/reject/revert, an explicit data refresh, capability-gap logging, progressive-disclosure reference docs) is gone from chat entirely — still reachable through the rest of this API, just not from this conversation. Ambiguity is handled by the model just asking in its plain-text reply; there's no dedicated clarifying-question tool anymore.

**Structurally guaranteed to never apply anything without manual approval (except unassign_task/create_task/complete_task/create_habit) — not just a prompt instruction.** There is no approve/reject/revert tool in this surface at all, so the model cannot apply a proposal even if asked to. `propose_calendar_change`, `assign_task_to_event`, and `plan_habits` additionally force `skipAutoApply: true` on every call (`lib/nlToolDispatch.ts`), which bypasses `AUTO_APPLY_CATEGORIES` entirely for these paths — **live-verified** for `propose_calendar_change`: with `AUTO_APPLY_CATEGORIES=personal` set, an identical `personal`-category proposal auto-applied immediately through the direct `POST /api/proposed-changes` call but stayed `pending` when created through chat. `assign_task_to_event`'s and `plan_habits`'s underlying `lib/aiTasks.ts`/`lib/habitPlacement.ts` functions didn't accept a `skipAutoApply` option at all before 2026-07-25 — closed as part of adding each tool, so the guarantee actually holds for every write tool that touches the calendar or review queue, not just the original one. `unassign_task`/`create_task`/`complete_task`/`create_habit` are the deliberate exceptions: none of them ever touch the calendar, only the task/habit lists, so they apply instantly like the direct API always has — `create_habit` was never gated even for the direct API (`backend-schema.md`'s `habits` entry: "a habit declaration is a goal, not a calendar write"), so there was no `AUTO_APPLY_CATEGORIES` gap to close for it in the first place.

**On caching, measured live (2026-07-25, at 9 tools):** the stable system-prompt prefix (behavioral rules + all tool schemas) is **3,589 tokens** (`anthropic.messages.countTokens`, real call). Claude Haiku 4.5's minimum cacheable-prefix length is **4,096 tokens** — so caching *still* doesn't trigger, even now, just under the line. This is the first time the exact current-model threshold has been checked against a live count (earlier notes only said "too small" without citing Haiku 4.5's specific number, which is higher than Sonnet/Fable's 2,048 or Sonnet 4.5-tier's 1,024 — see the claude-api skill's prompt-caching reference). Worth re-measuring after the next tool or system-prompt addition of any size — at ~88% of the threshold, it likely won't take much more to cross it, at which point the existing `cache_control` breakpoint starts working with zero code changes.

**Priority at confirmation time (2026-07-25):** `propose_calendar_change` and `assign_task_to_event`'s new-event mode both leave `priority` unset on the resulting proposal — the model can no longer set it (dropped from `propose_calendar_change`'s schema entirely; `assign_task_to_event` forces `carryOverPriority: false` rather than inheriting the task's own priority). `POST /api/proposed-changes/{id}/approve` now refuses to apply a `create` proposal that has a real `proposed_start`/`proposed_end` (a genuine calendar block, `lib/proposedChanges.ts`'s `isCalendarCreate`) while `priority` is still `null` — it fails with `status: "failed"` and an `error_message` telling you to `PATCH /api/proposed-changes/{id}` with a priority first, same ergonomics as a scheduling-rule violation or conflict (not a distinct HTTP error). This is scoped deliberately narrow: a `move` never sets/needs priority (it preserves the target event's own), and the task-list-intake `create` shape (no start/end, e.g. Todoist sync) is `tasks.priority`, a separate concern this check doesn't touch. Every other engine that creates a calendar block (Focus Time, habits, auto-reschedule, etc.) already sets a real priority at creation, so this gate never affects them in practice.
