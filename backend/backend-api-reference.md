...
**What it does:** Phase 5 — the NL chat layer. **Completely rebuilt 2026-07-25** (per an explicit user decision) from a 33-tool surface down to exactly **two abilities**, because the prior surface was too large for `claude-haiku-4-5` to reliably keep track of, then **grown to five abilities the same day** to add task assignment:
...
**Structurally guaranteed to never apply anything without manual approval (except unassign_task) — not just a prompt instruction.** There is no approve/reject/revert tool in this surface at all, so the model cannot apply a proposal even if asked to. `propose_calendar_change` and `assign_task_to_event` additionally force `skipAutoApply: true` on every call (`lib/nlToolDispatch.ts`), which bypasses `AUTO_APPLY_CATEGORIES` entirely for these paths — **live-verified** for `propose_calendar_change`: with `AUTO_APPLY_CATEGORIES=personal` set, an identical `personal`-category proposal auto-applied immediately through the direct `POST /api/proposed-changes` call but stayed `pending` when created through chat. `assign_task_to_event`'s underlying `lib/aiTasks.ts` functions didn't pass `skipAutoApply` at all before 2026-07-25 — closed as part of this same change, so the guarantee actually holds for both write tools, not just one. `unassign_task` is the one deliberate exception: it never touches the calendar, only the task's own status/link, so it applies instantly like the direct API always has.
