...
2. **`POST /api/chat`** — free text in, but the model has exactly **nine abilities** (grown from two, same day as the original rebuild — 2026-07-25): propose a calendar change (create/move/delete), find free time, list unscheduled tasks, assign a task to an event (link existing or create new), unassign a task, create a task (optionally with a deadline), complete a task, plan habits (auto-fill this period's remaining occurrences, spaced out), and declare a new habit. Nothing else — no editing/pausing an existing habit, scheduling rules, bulk operations, or approvals. This was a deliberate simplification (the prior ~31-tool surface was too large for `claude-haiku-4-5` to reliably track) — chat is now a narrow, reliable assistant, not a full alternate interface to everything in Section 5. A chat-originated proposal is still a completely ordinary `proposed_changes` row (the review-queue screen doesn't need a special code path for it) — it's the *tool surface* that shrank, not the underlying write path.

**Nothing from chat can ever apply itself, except unassigning/creating/completing a task or declaring a habit — this is structural, not a prompt instruction.** There is no approve/reject tool in chat's surface at all, and every write tool that touches the calendar or review queue (propose a change, assign a task, plan habits) forces a bypass of `AUTO_APPLY_CATEGORIES` (live-verified for the calendar-change path: an identical category that auto-applies through the direct API stays `pending` when created through chat). This was confirmed explicitly for habit planning too — the user was clear that must go through the same review tree as everything else, even though the direct `POST /api/habits/plan` endpoint doesn't force this itself. Unassigning/creating/completing a task and declaring a habit are the exceptions — none of them touch the calendar, only the task/habit lists, and all four apply instantly, matching how the direct API has always treated them (a habit *declaration* was never gated even for the direct API — it's a goal statement, not a calendar write, unlike planning its occurrences). Every other chat-created proposal needs the same manual approval tap through the app — always, not a v1 posture that might later change.

**The app needs to reload its task list when `tasks_changed` comes back `true`** (see the response shape below) — `create_task`/`unassign_task`/`complete_task` mutate the `tasks` table with no proposal to signal it through, unlike the calendar-change/assign-task tools' `proposals` field.

**Priority is never chat's call for a new calendar block (2026-07-25).** A chat-created event (via propose_calendar_change, or assign_task_to_event's new-event mode) always lands with `priority` unset — the model can't set it even if it tried, the field's gone from its tool schema. Approving that kind of proposal is blocked server-side until you set one via `PATCH /api/proposed-changes/{id}` — see Section 5's entry for that endpoint, and give the review-queue screen a way to set priority inline before/at the approve tap, not just display it read-only.
...
**`usage`** is on every response. `estimatedCostUsd` is a live estimate computed from the actual token counts against Anthropic's published per-token/cache-multiplier rates, not a billed total — good enough for an in-app "this conversation cost ~$0.003" indicator, not for financial reconciliation. If you want a running per-conversation total, sum it client-side across calls sharing a `conversation_id` — the backend doesn't persist or aggregate it anywhere today (only final message text is stored, see Section 4). **On caching, measured live (2026-07-25, at 9 tools):** the stable prompt prefix is 3,589 tokens — still under Claude Haiku 4.5's 4,096-token minimum cacheable length (higher than you might assume; Sonnet/Fable-tier models cache at 2,048 or even 1,024), so `cacheCreation*Tokens`/`cacheReadTokens` are still `0` on every call. It's close (~88% of the threshold) — check again with a real call after the next tool/prompt addition rather than assuming either way; see `backend-api-reference.md`'s `POST /api/chat` entry for the measurement technique.

**If you need chat to reach beyond these nine abilities** (e.g. "set up a recurring series," editing/pausing an existing habit), that's out of scope for `/api/chat` now — build a direct UI action against the relevant endpoint in Section 5 instead.
...
- **`tasks`** — one-off items with a single `deadline`. `status`: `unscheduled → scheduled → completed/discarded`, though `scheduled` isn't required to reach `completed` — `POST /api/tasks/{id}/complete` (added 2026-07-25) marks a task done directly from either `unscheduled` or `scheduled`, and never touches `scheduled_event_id` or the calendar event if it had one (the event stays as a historical record; this is a status flip, not an unschedule). `scheduled_event_id` links to the calendar event once placed — **many-to-one, not unique**: multiple tasks can share one block (e.g. several small tasks batched into one Focus Time slot). Query `GET /api/tasks?scheduled_event_id=...` to see everything attached to a given block; don't rely on the event's own title or metadata to tell you, since the event can only ever reflect one task's id and its title is never auto-managed. The event's real Google Calendar **description** is auto-kept in sync instead (added 2026-07-25, `lib/eventTaskDescription.ts`) — a generated, delimited section listing every currently-`scheduled` linked task (a completed one drops off automatically), updated whenever a link/unlink/complete actually applies. If the app ever renders a raw event description, expect that section to be present on any block with linked tasks; it's regenerated server-side, not something the app needs to build itself.

  **A task row can now disappear entirely, not just change status (added 2026-07-25, "Todoist is master" two-way sync)** — if a Todoist-tracked task vanishes from Todoist (checked off or deleted there), our row gets hard-`DELETE`d, not marked `'completed'`/`'discarded'`, once it's no longer active on our side. If the app ever caches a task list locally between refreshes, a task present in one fetch can legitimately be entirely absent from the next with no error — that's expected, not a sync bug to investigate.

  **`PATCH /api/tasks/{id}` now exists** (title/description/deadline/priority/duration_minutes/tags) — added specifically so two-way Todoist sync had something to push an edit *from*, not for the app yet; there's still no edit UI (`TaskListView` stays view/create/complete only per the app's own `CLAUDE.md`). Worth knowing if task-editing UI ever gets built here — the backend piece is already done.

  **Creating/editing/completing a task here now also pushes to Todoist, in real time** (`POST /api/tasks`, `PATCH /api/tasks/{id}`, `POST /api/tasks/{id}/complete` — all best-effort, a Todoist failure never blocks the app-facing response). Nothing the app needs to do differently — this is entirely a backend-side side effect, invisible in the response shape of any of those three endpoints.
...
| **Sync (individual, lower-level)** | `POST /api/calendar/sync`, `POST /api/calendar/sync/dedupe`, `POST /api/todoist/sync` | The individual syncs `POST /api/refresh` wraps — call these directly only if you need to retry/observe one source in isolation |
| **Force full calendar resync** | `POST /api/calendar/resync` | Added 2026-07-28 — the "resync calendars only" action the row above used to just gesture at. Clears every tracked calendar's `sync_token`/`page_token` and runs an immediate full backfill, rather than waiting for a calendar to naturally need one. This is what backs the Chrome extension's resync button (Swift app has no equivalent — no calendar-related UI exists there since its own calendar-grid removal). Same `truncatedByTimeBudget` resumability as `POST /api/calendar/sync` — a full resync across every calendar may take more than one call |
