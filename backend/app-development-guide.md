...
2. **`POST /api/chat`** — free text in, but the model has exactly **eight abilities** (grown from two, same day as the original rebuild — 2026-07-25): propose a calendar change (create/move/delete), find free time, list unscheduled tasks, assign a task to an event (link existing or create new), unassign a task, create a task (optionally with a deadline), complete a task, and plan habits (auto-fill this period's remaining occurrences, spaced out). Nothing else — no declaring/editing habits, scheduling rules, bulk operations, or approvals. This was a deliberate simplification (the prior ~31-tool surface was too large for `claude-haiku-4-5` to reliably track) — chat is now a narrow, reliable assistant, not a full alternate interface to everything in Section 5. A chat-originated proposal is still a completely ordinary `proposed_changes` row (the review-queue screen doesn't need a special code path for it) — it's the *tool surface* that shrank, not the underlying write path.

**Nothing from chat can ever apply itself, except unassigning/creating/completing a task — this is structural, not a prompt instruction.** There is no approve/reject tool in chat's surface at all, and every write tool that touches the calendar or review queue (propose a change, assign a task, plan habits) forces a bypass of `AUTO_APPLY_CATEGORIES` (live-verified for the calendar-change path: an identical category that auto-applies through the direct API stays `pending` when created through chat). This was confirmed explicitly for habits too — the user was clear that habit planning through chat must go through the same review tree as everything else, even though the direct `POST /api/habits/plan` endpoint doesn't force this itself. Unassigning, creating, and completing a task are the exceptions — none of them touch the calendar, only the task list, and all three apply instantly, matching how the direct API has always treated them. Every other chat-created proposal needs the same manual approval tap through the app — always, not a v1 posture that might later change.
...
**`usage`** is on every response. `estimatedCostUsd` is a live estimate computed from the actual token counts against Anthropic's published per-token/cache-multiplier rates, not a billed total — good enough for an in-app "this conversation cost ~$0.003" indicator, not for financial reconciliation. If you want a running per-conversation total, sum it client-side across calls sharing a `conversation_id` — the backend doesn't persist or aggregate it anywhere today (only final message text is stored, see Section 4). Note: right after the original 2-tool rebuild the stable prompt prefix was small enough (~1,576 tokens) that prompt caching didn't trigger at all; at 6 tools this hasn't been re-measured, so don't assume either way without checking `cacheCreation*Tokens`/`cacheReadTokens` on a real call.

**If you need chat to reach beyond these eight abilities** (e.g. "set up a recurring series," declaring a new habit), that's out of scope for `/api/chat` now — build a direct UI action against the relevant endpoint in Section 5 instead.
