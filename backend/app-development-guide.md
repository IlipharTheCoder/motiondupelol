...
2. **`POST /api/chat`** — free text in, but the model has exactly **six abilities** (grown from two, same day as the original rebuild — 2026-07-25): propose a calendar change (create/move/delete), find free time, list unscheduled tasks, assign a task to an event (link existing or create new), unassign a task, and create a task (optionally with a deadline). Nothing else — no habits, scheduling rules, bulk operations, or approvals. This was a deliberate simplification (the prior ~31-tool surface was too large for `claude-haiku-4-5` to reliably track) — chat is now a narrow, reliable assistant, not a full alternate interface to everything in Section 5. A chat-originated proposal is still a completely ordinary `proposed_changes` row (the review-queue screen doesn't need a special code path for it) — it's the *tool surface* that shrank, not the underlying write path.

**Nothing from chat can ever apply itself, except unassigning/creating a task — this is structural, not a prompt instruction.** There is no approve/reject tool in chat's surface at all, and every write tool that touches the calendar or review queue (propose a change, assign a task) forces a bypass of `AUTO_APPLY_CATEGORIES` (live-verified for the calendar-change path: an identical category that auto-applies through the direct API stays `pending` when created through chat). Unassigning and creating a task are the exceptions — neither touches the calendar, only the task list, and both apply instantly, matching how the direct API has always treated them. Every other chat-created proposal needs the same manual approval tap through the app — always, not a v1 posture that might later change.

**The app needs to reload its task list when `tasks_changed` comes back `true`** (see the response shape below) — `create_task`/`unassign_task` mutate the `tasks` table with no proposal to signal it through, unlike the calendar-change/assign-task tools' `proposals` field.
...
**If you need chat to reach beyond these six abilities** (e.g. "set up a recurring series," habits), that's out of scope for `/api/chat` now — build a direct UI action against the relevant endpoint in Section 5 instead.
