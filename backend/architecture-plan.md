...
The driver: of the two shapes considered for item 25 (native Google `recurrence`/RRULE vs. synthesizing N individual `create` proposals, "Option B"), Option B's weakness was having no way to treat "all N instances of one recurring series" as a single editable unit without a bespoke tracking table. Tags plus bulk-edit solve that generically instead — a recurring series tags every instance with a shared `series:<uuid>`, and "edit/cancel the whole series" becomes an ordinary bulk-edit-by-tag call.

**Item 25 built (2026-07-24), Option B confirmed.** `POST /api/calendar/recurring` (`lib/recurringEvents.ts`'s `planRecurringSeries`) generates N weekly-cadence occurrences and creates one ordinary `create` proposal per occurrence, each tagged with a fresh `series:${randomUUID()}` (Node's built-in `crypto.randomUUID()` — no new dependency) plus any caller-supplied tags. No new tracking table — the tag *is* the series record, exactly as planned. Each occurrence goes through the exact same per-instance `detectConflicts` check at apply time as any other `create` (`applyProposedChange`, unchanged) — this is the other half of why Option B was chosen: no new batch/series conflict-checking logic needed, one occurrence's conflict just fails that one proposal without blocking the rest.

Date generation (`lib/recurringOccurrences.ts`'s `generateWeeklyOccurrences`, kept in its own side-effect-free file — mirrors `lib/habitSpacing.ts` vs. `lib/habitPlacement.ts`'s split, so it's unit-testable without pulling in `lib/proposedChanges.ts`'s Supabase client) supports weekly cadence with a configurable `interval_weeks` (1 = every week, 2 = every other week), anchored to the first occurrence's own weekday and time-of-day. Interval math runs through Luxon's zone-aware `plus({ weeks })` in `HOME_TIMEZONE`, not raw millisecond arithmetic, so "7pm every Thursday" stays 7pm local across a DST transition between occurrences — unit-tested against both the spring-forward and fall-back transitions, same DST-correctness bar `lib/workingHours.ts` already meets. Scope confirmed with the user: single-weekday, weekly-interval patterns only for v1 (no monthly/multi-weekday-per-series — habits already own the monthly-goal shape; multi-weekday recurring *events* specifically weren't a stated need).

**Confirmed with the user — the exact question this item's own notes flagged as unsolved ("how far ahead do we generate, and who re-generates more as time passes"):** `count` or `until` is *required* — no default horizon. This was a deliberate tradeoff: it means there's no "silently ran out" failure mode and no "top up" mechanism needed at all (a still-wanted indefinite series is just a matter of calling the endpoint again later), at the cost of never being able to say "forever" in one call. A hard `MAX_OCCURRENCES = 260` (5 years weekly) sanity-caps any single request regardless; a `truncated: true` response field surfaces if an `until`-bounded request would have exceeded it.
